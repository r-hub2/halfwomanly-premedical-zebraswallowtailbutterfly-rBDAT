#' @title Calculate wood volume over bark of a tree between height A and B
#'
#' @description BDAT-Function to get wood volume over bark of one or many trees
...
#' @seealso \code{\link{BDATVOLABOR}} for BDAT routine calculating volume under
#' bark, \code{\link{getVolume}} for a function with a convenient english name,
#' more options and a bark switch.
#' @export
BDATVOLABMR <- function(BDATArtNr, D1, H1 = 1.3, D2 = 0, H2 = 0, H, A, B,
                        SekLng = 2.0) {
  df <- data.frame(
    spp = BDATArtNr, D1 = D1, H1 = H1,
    D2 = D2, H2 = H2, H = H, A = A, B = B, sl = SekLng
  )
  res <- getVolume(tree = df, iAB = "H", bark = T, mapping = NULL)
  return(res)
}
